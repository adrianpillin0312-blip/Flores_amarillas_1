...
var doc=document,flower=doc.querySelector('.flower'),petalPartMarkup='<div class="box"><div class="shape"></div></div>',maxParts=20,maxPetals=6,partsFontStep=25/maxParts;createFlower();function createFlower(){var angle=360/maxPetals;for(var i=0;i<maxPetals;i++){var petal=createPetal(),currAngle=angle*i+'deg',transform='transform: rotateY('+currAngle+') rotateX(-30deg) translateZ(9vmin)';petal.setAttribute('style',transform);flower.appendChild(petal);}}function createPetal(){var box=createBox(null,0),petal=doc.createElement('div');petal.classList.add('petal');for(var i=1;i<=maxParts;i++){box=createBox(box,i);}petal.appendChild(box);return petal;}function createBox(box,pos){var fontSize=partsFontStep*(maxParts-pos)+'vmin',half=maxParts/2,bright='50';if(pos<half+1){fontSize=partsFontStep*pos+'vmin';}else{bright=10+40/half*(maxParts-pos);}var baseHue=50,hueVariation=10,saturation=70+(20*pos/maxParts),color='hsl('+(baseHue+(hueVariation*pos/maxParts))+', '+saturation+'%, '+bright+'%)',newShape=doc.createElement('div');newShape.classList.add('shape');var newBox=doc.createElement('div');newBox.classList.add('box');newBox.setAttribute('style','color: '+color+';font-size: '+fontSize);if(box)newBox.appendChild(box);newBox.appendChild(newShape);return newBox;}function drawGalaxy(){var canvas=document.getElementById('galaxy-canvas');if(!canvas)return;var ctx=canvas.getContext('2d');function resize(){canvas.width=window.innerWidth;canvas.height=window.innerHeight;}resize();window.addEventListener('resize',resize);var stars=[],numStars=120;for(var i=0;i<numStars;i++){stars.push({x:Math.random()*canvas.width,y:Math.random()*canvas.height,r:Math.random()*1.5+0.5,dx:(Math.random()-0.5)*0.7,dy:(Math.random()-0.5)*0.7,alpha:Math.random()*0.5+0.5});}function animate(){ctx.clearRect(0,0,canvas.width,canvas.height);for(var i=0;i<stars.length;i++){var s=stars[i];ctx.save();ctx.globalAlpha=s.alpha;ctx.beginPath();ctx.arc(s.x,s.y,s.r,0,Math.PI*2);ctx.fillStyle='rgba(255,182,193,0.9)';ctx.shadowColor='#ffb6d5';ctx.shadowBlur=2;ctx.fill();ctx.restore();s.x+=s.dx;s.y+=s.dy;if(s.x<0||s.x>canvas.width)s.dx*=-1;if(s.y<0||s.y>canvas.height)s.dy*=-1;}requestAnimationFrame(animate);}animate();}document.addEventListener('DOMContentLoaded',drawGalaxy);
  ...
  const messages = [
    'Quiero que este día sea más',
    'Que celebrar el inicio de primavera,',
    'Quiero que sea celebrar',
    'Muchos más días con mi princesa,',
    'Que veamos juntos el mar',
    'Y que estemos la vida entera',
    'Enamorados hasta el final,',
    'Que sea un amor sin fronteras.',
    'Quiero dedicarte esta flor,',
    'Capaz no lo ves como mucho',
    'Pero es una muestra de mi amor,',
    'Que veas que es profundo,',
    'Eres dueña de mi corazón',
    'Y eres lo mejor de mi mundo.',
    'Quiero que pasemos un verano',
    'Juntos, en una piscina',
    'estar mucho a tu lado',
    'Y que todo sea risas,',
    'Quedarme dándote abrazos',
    'Ver lo que me hipnotiza,',
    'Tus lindos ojos y cabello,',
    'Tus lindos labios y sonrisa.',
    'Si tuviera que expresar',
    'Que es lo que mas me gusta',
    'Sería tu rostro espectacular',
    'Que no verlo siempre me frustra,',
    'Le pido siempre a la luna',
    'Que te cuide ya que como tu ninguna.',
    'Quiero estar junto a ti en otoño,',
    'Agarrarte de la mano y darte',
    'Mi corazón atado a un moño,',
    'Que seamos los dos parte',
    'De una historia, en un entorno',
    'Lleno de amor eterno y arte,',
    'Donde somos tu y yo, solo nosotros.',
    'Daría tanto por ti que cada',
    'Cosa se ve pequeña,',
    'Quiero tener una casa',
    'Y tener miles de cuadros en ella,',
    'Que cada uno sea de tu cara',
    'Y de tu gran belleza.',
    'Me imagino un futuro',
    'Contigo en invierno',
    'Donde nuestro amor puro',
    'Caliente los fríos vientos,',
    'Tomando café y siendo tiernos,',
    'Estando los dos seguros',
    'Y juntos cada momento.',
    'Eres la mujer de mi vida',
    'Nadie quitara esa idea',
    'Pues tu eres mi sonrisa',
    'Eres de cualquiera la ideal',
    'Por ti el mundo recorrería de ida',
    'Y vuelta, pareces irreal.',
    'Att: Adrián',
    'Para: Ashley',
